function questionsOnly_qbu(command_str,position,file_path)
...
if ~strcmp(command_str,'initialize')
    ...
    if ~strcmp(get(h_fig,'tag'),'questions_fig')
        ...
        if isempty(h_fig)
            ...
            questionsOnly_qbu('initialize'); %first initialize
            questionsOnly_qbu(command_str); %then rin the required command
            return;
        end
    end
end
...
if strcmp(command_str,'initialize')
    ...
    % Check if open questions are present and give focus to text field
    openquestion(1) = findobj(h_fig,'tag','question_box1');
    openquestion(2) = findobj(h_fig,'tag','question_box2');
    openquestion(3) = findobj(h_fig,'tag','question_box3');
    openquestion(4) = findobj(h_fig,'tag','question_box4');
    
    i = 1;
    while i < length(openquestion)
        openpanel = get(openquestion(i),'parent');
        if strcmp(get(openpanel,'visible'),'on') == 1
            setfocus(openquestion(i));
            i = length(openquestion);
        end
        i = i + 1;
    end
    
    % Wait here until continue button is pressed
    
    %setfocus(findobj(h_fig,'tag','continue_button'));
